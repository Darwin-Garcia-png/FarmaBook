...
class AlertasScreen extends StatelessWidget {
        ...
        return Scaffold(
          ...
          appBar: PremiumHeader(
            ...
            leading: GoRouter.maybeOf(context)?.canPop() == true
                ? IconButton(
                    icon: const Icon(Icons.arrow_back_rounded, color: AppTheme.ayanamiBlue),
  ...
  Widget _buildAlertCard(BuildContext context, Map<String, dynamic> n, LotesController lotesCtrl, DashboardController dashCtrl) {
    final notifCtrl = Provider.of<NotificacionesController>(context, listen: false);
    ...
    final fecha = n['fecha'] ?? '';
    final nId = (n['notificacionId'] ?? n['id'] ?? '').toString();
    final read = notifCtrl.isRead(n);
    ...
    return Container(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(32),
        onTap: () {
            if (!read && nId.isNotEmpty) notifCtrl.markAsRead(nId);
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              if (!read)
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: AppTheme.ayanamiBlue.withOpacity(0.4), blurRadius: 6)],
                  ),
                ),
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                  color: read ? color.withOpacity(0.05) : color.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(20),
                ),
                child: Icon(isUrgent ? Icons.inventory_2_rounded : Icons.event_busy_rounded,
                    color: read ? color.withOpacity(0.4) : color, size: 28),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(isUrgent ? 'STOCK CRÍTICO' : 'VENCIMIENTO PRÓXIMO',
                      style: TextStyle(
                        ...
                        color: read ? color.withOpacity(0.4) : color,
                        letterSpacing: 1.5)),
                    ...
                    Row(
                      children: [
                        Icon(Icons.access_time_rounded, size: 12,
                            color: read ? Colors.grey.shade300 : Colors.grey.shade400),
                        const SizedBox(width: 6),
                        Text(fecha, style: TextStyle(
                            fontSize: 11,
                            color: read ? Colors.grey.shade300 : Colors.grey.shade500,
              ...
              if (!read)
                IconButton(
                  icon: Icon(Icons.check_circle_outline, color: AppTheme.greenMetal, size: 22),
                  onPressed: () {
                    if (nId.isNotEmpty) notifCtrl.markAsRead(nId);
                  },
                  tooltip: 'Marcar como leída',
                ),
              // Delete button
              IconButton(
                icon: Icon(Icons.delete_outline_rounded, color: AppTheme.reiOrangeRed.withOpacity(0.6), size: 22),
                onPressed: () {
                  if (nId.isNotEmpty) notifCtrl.deleteNotification(nId);
                },
